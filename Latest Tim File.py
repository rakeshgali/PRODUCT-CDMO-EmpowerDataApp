...
if stop_program == False:
    ...
    file_sample_list_final = sorted(set(file_sample_list), key=lambda x: x[1]) # Sort by sample name and remove duplicates when both sample name and file name are the same
    Name_RRT_list_final = sorted(set(Name_RRT_list), key=lambda x: x[1]) # Sort by RRT and remove duplicates when both peak name and RRT are the same
            ...
            for row in csv_reader:
                ...
                if line_number > 2:
                    ...
                    if AN != '':
                        if float(AN) < float(QL):
                            AN = ''
                            if RRT != '':
                                if float(RRT) == 1:  # handles special condition where after QL is applied, AN value for RRT = 1 becomes ND
               ...
               ##### v8 code #####
                        ...
                        if Name == Name_ws and AN != '' and RRT != '': # if Peak Name from compiled table matches that from each sample
                            if new_ws.cell(row=y, column=x).value != None:  # Deal with more than one value occupying same cell and highlight in red font
                                ...
                                if type(existing_value) == float:
                                    existing_value = format(existing_value, '.2f')
                                overlap_value = float(AN)
                                overlap_value = format(overlap_value, '.2f')
                                ...
                                new_ws.cell(row=y, column=x).value = float(AN)  # overwrite ND with AN value in number format if it exists
                                ...
                                if l_max_exist == True and existing_l_max_value == None:
                                    ...
                                    new_ws.cell(row=2, column=x).value = float(l_max)  # fill in first row with lamda max information
    ...
    for peak_index in range(peak_number):
        ...
        if new_ws.cell(row=3, column=x_header).value == new_ws.cell(row=3, column=x_header-1).value:  # Highlight peak with same RRT; help user tidy up data
            # new_ws.cell(row=3, column=x).font = Font(color="00FF6600")
        ...
        if file_name in Do_Not_Overwrite_list:  # handles special condition where after QL is applied, AN value for RRT = 1 becomes ND
            with open(file_name) as csv_file:
                ...
                for row in csv_reader:
                    if line_number > 2:
                        Name = row[0]
                        RRT = row[1]  # handles special condition where after QL is applied, AN value for RRT = 1 becomes ND
                        AN = row[2]
                        x = 3
                        if RRT != '':
                                ...
                                if AN != '' and RT != '':
                                    ...
                                    while x < excel_RT_count+3 and AN_assigned == False:
                                        ...
                                        if AN_assigned == False and x == excel_RT_count+3:
                                            ...
                                            if lambda_max != '':
                                                comment = 'RT ' + str(RT) + ': ' + str(AN) + ' (' + str(lambda_max) + ' nm)'
                                            else:
    ...
    new_ws.cell(row=y, column=1).value = '  [' + str(tabulated_file_count) + '/' + str(file_count) + ' files tabulated by RRT]'
    if invalid_file_count != 0:
        ...
        new_ws.cell(row=y+1, column=1).value = '  [Check data for file highlighted in RED. AN value cannot be assigned based on RRT; no AN value for RRT = 1]'
        if Unassigned_data == True:
            new_ws.cell(row=y+2, column=1).value = '  [See Comment Column for AN value that cannot be assigned based on Peak Name (row 4), RT or RT range (row 1)]'
    ...
    ws2.cell(row=4, column=ws2_dimen_x+4).comment = Comment('Unable to assign AN value based on RRT and Peak Name; no value for RRT = 1.  Assignment to the table was attempted by Data_Compiler if the peak satisfied both criteria of matching RT/RT range (row 1) AND Lambda Max (row 2).', 'Timothy Chang')
    ...
    while y < table_dimen_y+5:
        ...
        font_color = new_ws.cell(row=y, column=2).font.color.rgb  # Change sample name and file name to red if no AN value for RRT = 1
            ...
            if peak_value != None:
                if ws2.cell(row=y, column=ws2_dimen_x+3).value == None:
                    if type(peak_value) == str:  # handles special case where multiple values in one AN value cell in main table
                        ...
                        ws2.cell(row=y, column=ws2_dimen_x+3).alignment = Alignment(wrapText=True, horizontal='center')
                    else:
                        peak_value = format(peak_value, '.2f')
                        ws2.cell(row=y, column=ws2_dimen_x+3).value = 'RRT ' + str(RRT_header) + ': ' + str(peak_value)
                        ws2.cell(row=y, column=ws2_dimen_x+3).alignment = center_aligned_text
