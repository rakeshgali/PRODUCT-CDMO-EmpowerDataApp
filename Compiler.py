...
def process_file(reader_list, QL):

    for (file_name, file_reader) in reader_list:

        # this check can happen in the app
        if file_name == 'error_message.txt':
    ...
    if 1:  #stop_program == False:
        for (file_name, csv_rows) in reader_list:
            line_number = 0  # First line of txt file.
            # with open(os.path.join('data', file)) as csv_file:
            if 1:  # with csv_reader:
                        ...
                        for peak_index in range(peak_number):
                            ...
                            if Name == Name_ws and AN != '' and RRT != '':  # if Peak Name from compiled table matches that from each sample
                                if new_ws.cell(row=y,
                                    ...
                                    if type(existing_value) == float: # changed on 06/22/2021
                                        existing_value = format(existing_value, '.2f') # changed on 06/22/2021
                                    overlap_value = float(AN)
                                    overlap_value = format(overlap_value, '.2f') # changed on 06/22/2021
        ...
        for i in range(file_count):
            ...
            if file_name in Do_Not_Overwrite_list:  # handles special condition where after QL is applied, AN value for RRT = 1 becomes ND
                ...
                if 1:
                    # csv_reader = csv.reader(csv_file, delimiter='\t')
                    line_number = 0
                    for row in csv_rows:  #csv_reader:
                        if line_number > 2:
                            Name = row[0]
                            RRT = row[
                ...
                for i_invalid in range(invalid_file_count):
                    if file_name == invalid_file_list[i_invalid]:
                        if 1:  #with open(file_name,'rb') as csv_file:
                            # csv_reader = csv.reader(csv_file, delimiter='\t',doublequote= False)
                            line_number = 0  # First line of txt file.
                            for row in csv_rows: #csv_reader:
                                if line_number == 2:  # Count number of column in txt file
                                    column_count = len(row)
                                ...
                                line_number += 1
                y += 1
            #else:  # handles special condition where after QL is applied, AN value for RRT = 1 becomes ND # changed on 06/22/2021
            #   y += 1 # changed on 06/22/2021
                # End of this section
...
if __name__ == '__main__':
    filename1 = r'C:\Rakesh Gali\Personal\Python\OneDrive_2021-06-14\Demo 11Jun2021\TXT_export4070.txt'
    filename2 = r'C:\Rakesh Gali\Personal\Python\OneDrive_2021-06-14\Demo 11Jun2021\TXT_export4069.txt'
    filename3 = r'C:\Rakesh Gali\Personal\Python\OneDrive_2021-06-14\Demo 11Jun2021\TXT_export4068.txt'

   # reader_list= pd.DataFrame([['RRT~ 3.742',	'3.74',	'0.08',	'210.1','17.39']])
    ...
    for file_name in file_list:
        ...
        reader_list += [(file_name, csv_rows)]
    df = process_file(reader_list, 0.1)
